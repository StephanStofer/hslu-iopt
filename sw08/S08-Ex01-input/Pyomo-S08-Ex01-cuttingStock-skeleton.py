...
model.x = pyo.Var(patternIds, within=pyo.NonNegativeIntegers)


# constraints
def demand_constraint(model, b):
    return sum(patterns[p][b] * model.x[p] for p in patternIds) >= demands[b]


model.demand_constraints = pyo.Constraint(breadth, rule=demand_constraint)

# objective
model.obj = pyo.Objective(expr=sum(model.x[p] for p in patternIds), sense=pyo.minimize)
